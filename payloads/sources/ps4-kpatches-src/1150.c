...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x6278b4, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x2bd2ed, 0xeb); // bzero
    write8(kbase, 0x2bd331, 0xeb); // pagezero
    write8(kbase, 0x2bd3ad, 0xeb); // memcpy
    write8(kbase, 0x2bd3f1, 0xeb); // pagecopy
    write8(kbase, 0x2bd59d, 0xeb); // copyin
    write8(kbase, 0x2bda4d, 0xeb); // copyinstr
    write8(kbase, 0x2bdb1d, 0xeb); // copystr

    // stop sysVeri from causing a delayed panic on suspend
    write16(kbase, 0x62819f, 0x00eb);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x3bd620, 0xc3c03148);
    ...
    write8(kbase, 0x1fa63a, 0x37);
    write8(kbase, 0x1fa63d, 0x37);
    ...
    write64(kbase, sysent_11_off + 8, kbase + 0x704d5);
