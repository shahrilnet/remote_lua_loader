...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x1B76E3, 0x04eb); // skip check 1
    write16(kbase, 0x1B76F3, 0x04eb); // skip check 2
    write16(kbase, 0x1B7713, 0xe990); // nop + jmp

    // ChendoChap's patches from pOOBs4
    write16(kbase, 0x627B84, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x2BD42D, 0xeb); // bzero
    write8(kbase, 0x2BD471, 0xeb); // pagezero
    write8(kbase, 0x2BD4ED, 0xeb); // memcpy
    write8(kbase, 0x2BD531, 0xeb); // pagecopy
    write8(kbase, 0x2BD6DD, 0xeb); // copyin
    write8(kbase, 0x2BDB8D, 0xeb); // copyinstr
    write8(kbase, 0x2BDC5D, 0xeb); // copystr
    ...
    // }
    write16(kbase, 0x2FC14C, 0x04eb);

    // TODO: Description of this patch. patch sys_dynlib_load_prx()
    write16(kbase, 0x1B71A4, 0xe990);
    ...
    // cmp     eax, 0x4000000
    // jb      ... ; patch jb to jmp
    write32(kbase, 0x1B7758, 0x013ce990);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x3BD8C0, 0xc3c03148);
    ...
    write8(kbase, 0x1FA77A, 0x37);
    write8(kbase, 0x1FA77D, 0x37);
