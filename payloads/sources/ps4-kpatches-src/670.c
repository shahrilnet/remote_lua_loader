...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x63c8ce, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x3c14fd, 0xeb); // bzero
    write8(kbase, 0x3c1541, 0xeb); // pagezero
    write8(kbase, 0x3c15bd, 0xeb); // memcpy
    write8(kbase, 0x3c1601, 0xeb); // pagecopy
    write8(kbase, 0x3c17ad, 0xeb); // copyin
    write8(kbase, 0x3c1c5d, 0xeb); // copyinstr
    write8(kbase, 0x3c1d2d, 0xeb); // copystr

    // stop sysVeri from causing a delayed panic on suspend
    write16(kbase, 0x63d1cf, 0x00eb);
    ...
    // }
    write16(kbase, 0x451db8, 0x04eb);

    // TODO: Description of this patch. patch sys_dynlib_load_prx()
    write16(kbase, 0x1d83ce, 0xe990);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x41a2d0, 0xc3c03148);
    ...
    // }
    const u64 sysent_11_off = 0x111e210;
    // .sy_narg = 2
    write32(kbase, sysent_11_off, 2);
    // .sy_call = gadgets['jmp qword ptr [rsi]']
    write64(kbase, sysent_11_off + 8, kbase + 0x9d11d);
