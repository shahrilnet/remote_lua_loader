...
static inline void do_patch(void *kbase) {
    ...
    write8(kbase, 0xaed, 0xeb); // bcopy
    write8(kbase, 0x40580d, 0xeb); // bzero
    write8(kbase, 0x405851, 0xeb); // pagezero
    write8(kbase, 0x4058cd, 0xeb); // memcpy
    write8(kbase, 0x405911, 0xeb); // pagecopy
    write8(kbase, 0x405abd, 0xeb); // copyin
    write8(kbase, 0x405f6d, 0xeb); // copyinstr
    write8(kbase, 0x40603d, 0xeb); // copystr
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x400030, 0xc3c03148);
    ...
    write8(kbase, 0x3c24d9, 0x37);
    write8(kbase, 0x3c24dc, 0x37);
    ...
    // }
    const u64 sysent_11_off = 0x1115ed0;
    // .sy_narg = 2
    write32(kbase, sysent_11_off, 2);
    // .sy_call = gadgets['jmp qword ptr [rsi]']
    write64(kbase, sysent_11_off + 8, kbase + 0xaf8c);
