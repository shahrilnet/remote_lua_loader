...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x627B34, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x2BD40D, 0xeb); // bzero
    write8(kbase, 0x2BD451, 0xeb); // pagezero
    write8(kbase, 0x2BD4CD, 0xeb); // memcpy
    write8(kbase, 0x2BD511, 0xeb); // pagecopy
    write8(kbase, 0x2BD6D6, 0xeb); // copyin
    write8(kbase, 0x2BDB86, 0xeb); // copyinstr
    write8(kbase, 0x2BDC3D, 0xeb); // copystr

    // stop sysVeri from causing a delayed panic on suspend
    write16(kbase, 0x62841F, 0x00eb);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x3BD8A0, 0xc3c03148);
    ...
    write8(kbase, 0x1FA75A, 0x37);
    write8(kbase, 0x1FA75D, 0x37);
