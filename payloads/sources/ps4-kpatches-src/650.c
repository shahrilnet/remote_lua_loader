...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x63c50e, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x3c114d, 0xeb); // bzero
    write8(kbase, 0x3c1191, 0xeb); // pagezero
    write8(kbase, 0x3c120d, 0xeb); // memcpy
    write8(kbase, 0x3c1251, 0xeb); // pagecopy
    write8(kbase, 0x3c13fd, 0xeb); // copyin
    write8(kbase, 0x3c18ad, 0xeb); // copyinstr
    write8(kbase, 0x3c197d, 0xeb); // copystr

    // stop sysVeri from causing a delayed panic on suspend
    write16(kbase, 0x63ce0f, 0x00eb);
    ...
    // }
    write16(kbase, 0x451a08, 0x04eb);

    // TODO: Description of this patch. patch sys_dynlib_load_prx()
    write16(kbase, 0x1d801e, 0xe990);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x419f20, 0xc3c03148);
    ...
    write8(kbase, 0xab57a, 0x37);
    write8(kbase, 0xab57d, 0x37);
    ...
    // }
    const u64 sysent_11_off = 0x111d210;
    // .sy_narg = 2
    write32(kbase, sysent_11_off, 2);
    // .sy_call = gadgets['jmp qword ptr [rsi]']
    write64(kbase, sysent_11_off + 8, kbase + 0x15a50d);
