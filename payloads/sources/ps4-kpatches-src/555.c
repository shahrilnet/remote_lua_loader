...
static inline void do_patch(void *kbase) {
    ...
    write8(kbase, 0x405bcd, 0xeb); // bzero
    write8(kbase, 0x405c11, 0xeb); // pagezero
    write8(kbase, 0x405c8d, 0xeb); // memcpy
    write8(kbase, 0x405cd1, 0xeb); // pagecopy
    write8(kbase, 0x405e7d, 0xeb); // copyin
    write8(kbase, 0x40632d, 0xeb); // copyinstr
    write8(kbase, 0x4063fd, 0xeb); // copystr
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x4003F0, 0xc3c03148);
    ...
    write8(kbase, 0x3c2899, 0x37);
    write8(kbase, 0x3c289c, 0x37);
    ...
    // }
    const u64 sysent_11_off = 0x111aed0;
    // .sy_narg = 2
