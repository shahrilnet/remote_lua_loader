...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x1B76F3, 0x04eb); // skip check 1
    write16(kbase, 0x1B7703, 0x04eb); // skip check 2
    write16(kbase, 0x1B7723, 0xe990); // nop + jmp

    // ChendoChap's patches from pOOBs4
    write16(kbase, 0x627B84, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x2BD43D, 0xeb); // bzero
    write8(kbase, 0x2BD481, 0xeb); // pagezero
    write8(kbase, 0x2BD4FD, 0xeb); // memcpy
    write8(kbase, 0x2BD541, 0xeb); // pagecopy
    write8(kbase, 0x2BD6ED, 0xeb); // copyin
    write8(kbase, 0x2BDB9D, 0xeb); // copyinstr
    write8(kbase, 0x2BDC6D, 0xeb); // copystr

    // stop sysVeri from causing a delayed panic on suspend
    write16(kbase, 0x62846F, 0x00eb);
    ...
    // }
    write16(kbase, 0x2FC15C, 0x04eb);

    // TODO: Description of this patch. patch sys_dynlib_load_prx()
    write16(kbase, 0x1B71B4, 0xe990);
    ...
    // cmp     eax, 0x4000000
    // jb      ... ; patch jb to jmp
    write32(kbase, 0x1B7768, 0x013ce990);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x3BD8D0, 0xc3c03148);
    ...
    write8(kbase, 0x1FA78A, 0x37);
    write8(kbase, 0x1FA78D, 0x37);
