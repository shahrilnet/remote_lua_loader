...
static inline void do_patch(void *kbase) {
    ...
    write16(kbase, 0x1B7703, 0x04eb); // skip check 1
    write16(kbase, 0x1B7713, 0x04eb); // skip check 2
    write16(kbase, 0x1B7733, 0xe990); // nop + jmp

    // ChendoChap's patches from pOOBs4
    write16(kbase, 0x627FC4, 0x00eb); // veriPatch
    write8(kbase, 0xacd, 0xeb); // bcopy
    write8(kbase, 0x2BD44D, 0xeb); // bzero
    write8(kbase, 0x2BD491, 0xeb); // pagezero
    write8(kbase, 0x2BD50D, 0xeb); // memcpy
    write8(kbase, 0x2BD551, 0xeb); // pagecopy
    write8(kbase, 0x2BD6FD, 0xeb); // copyin
    write8(kbase, 0x2BDBAD, 0xeb); // copyinstr
    write8(kbase, 0x2BDC7D, 0xeb); // copystr

    // stop sysVeri from causing a delayed panic on suspend
    write16(kbase, 0x6288AF, 0x00eb);
    ...
    // }
    write16(kbase, 0x2FC4AC, 0x04eb);

    // TODO: Description of this patch. patch sys_dynlib_load_prx()
    write16(kbase, 0x1B71C4, 0xe990);
    ...
    // cmp     eax, 0x4000000
    // jb      ... ; patch jb to jmp
    write32(kbase, 0x1B7778, 0x013ce990);
    ...
    //     mov     rbp, rsp
    //     ...
    write32(kbase, 0x3BDD10, 0xc3c03148);
    ...
    write8(kbase, 0x1FA79A, 0x37);
    write8(kbase, 0x1FA79D, 0x37);
    ...
    write64(kbase, sysent_11_off + 8, kbase + 0x47b31);
